/* ============================================================
   MercuriScan — logique applicative
   ============================================================ */
(function(){
"use strict";
...
var WATER_L=2;                                 // litres/jour (hypothèse)
...
var A={portion:200, meals:7, weight:60};       // hypothèses d'exposition
var sel=0;                                      // site sélectionné
var scen="statuquo";
...
function ensureMap(){
  ...
  if(!mapInited){
    ...
    LMAP.attributionControl.setPrefix("Leaflet");   // retire le drapeau 🇺🇦 par défaut
...
function focusNode(gn){
  ...
  for(var k in netEls){ if(k[0]==="_")continue; var e=netEls[k];
...
function parseCSV(text){
  ...
  return lines.slice(1).map(function(l){
    var c=l.split(",");
    return {nom:c[idx.nom_site]||"?", lat:num(c[idx.latitude]), lon:num(c[idx.longitude]),
...
function initCSV(){
  ...
  inp.addEventListener("change",function(){
    ...
    rd.onload=function(){ try{ var rows=parseCSV(rd.result);
...
function initControls(){
  ...
  document.querySelectorAll("#scenTabs button").forEach(function(b){
    b.addEventListener("click",function(){ scen=b.dataset.s;
